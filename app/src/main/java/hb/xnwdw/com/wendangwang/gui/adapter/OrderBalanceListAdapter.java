...
//            ((TextView)(helper.getView(R.id.presentnum_tv))).setText("×"+item.getPresentItemList().get(0).getPresentNum()+"");
            ...
            LinearLayout infoLl = helper.getView(R.id.present_ll);

            for (int i = 0; i < item.getPresentItemList().size(); i++) {
                LinearLayout ll = new LinearLayout(context);
                LinearLayout.LayoutParams pamr = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                pamr.setLayoutDirection(LinearLayout.HORIZONTAL);
                ll.setLayoutParams(pamr);
                TextView tvName = new TextView(context);
                tvName.setText(item.getPresentItemList().get(i).getItemName());
                tvName.setTextSize(10);
                tvName.setTextColor(context.getResources().getColor(R.color.black));
                TextView tvNum = new TextView(context);
                tvNum.setText("×" + item.getPresentItemList().get(i).getPresentNum());
                tvNum.setTextSize(10);
                tvNum.setTextColor(context.getResources().getColor(R.color.maincolor));
                ll.addView(tvName);
                ll.addView(tvNum);
                infoLl.addView(ll);
            }
